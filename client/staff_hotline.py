...
router.message.filter(ChatTypeFilter(chat_type="supergroup"), IsStaffFilter()) # применение фильтра супергрупп к роутеру

content_types = ("text", "sticker", "animation", "photo", "video", "voice", "video_note", "contact", "location", "venue", "document")
...
async def answer_to_clt(message: types.Message):
    ...
    try:
        chat_id = int([chat_id for chat_id, thread_id in hotline_chat.items() if thread_id == message.message_thread_id][0])
    ...
    await bot.edit_message_reply_markup(chat_id=chat_id, message_id=inline_keyboard_msg_id[chat_id], reply_markup=None)	
    # удаление инлайн клавиатуры с последнего сообщения в чате клиента с ботом

    if content_type == "text":
        content = message.md_text
        htln_message = await eval(code_to_execute.format("message", "text=content", chat_id))
